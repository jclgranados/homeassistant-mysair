...
| C4 | `translations/` + `strings.json` (config flow, servicios). | ✅ Hecho — `strings.json` (inglés, referencia/fallback) + `translations/es.json`; cubre pasos del config flow (`user`/`reauth_confirm`), errores, abort reasons y el servicio `mysair.stop_installation`. **Actualización (2026-09-12):** `translations/en.json` faltaba, y HA **no lee `strings.json` en runtime** para integraciones custom — los usuarios en inglés veían claves crudas. Añadido. `has_entity_name` sí se adoptó finalmente, forzado por el cambio de composición de `entity_id` en HA 2026 (ver `execution-plan.md` Tarea 33); los nombres de entidad siguen fijos en español, sin `translation_key`. |
