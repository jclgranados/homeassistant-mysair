...
- **Hallazgo de ecosistema:** PyPI no publica `homeassistant` más reciente que `2025.1.4` (más antiguo que el `2025.10.0` mínimo declarado en `manifest.json`); `pytest-homeassistant-custom-component==0.13.205` (la más reciente en PyPI) fija esa versión internamente vía `Requires-Dist`. Las APIs usadas por el código (`ConfigEntryAuthFailed`, `_get_reauth_entry`, `async_update_reload_and_abort`, `async_set_unique_id`) ya existían en 2025.1.4, así que los tests son válidos aunque no cubren la versión mínima exacta declarada.
