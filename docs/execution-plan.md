...
- **Otros campos observados en la captura real, no tratados en esta tarea** (documentados como observación, no como bug): `vf`, `hmh`, `mh`, `p`, `ps`, `pl`, `sv` aparecen en el JSON de zona sin mapear a nada — no rompen nada (se ignoran), pero quedan como posible trabajo futuro si resulta que faltan datos relevantes.
...
- Campos de zona sin mapear observados en producción (`vf`, `hmh`, `mh`, `p`, `ps`, `sv`) — investigados sin resultado en el bundle JS (Tarea 22); solo se resolverían con una captura adicional o el bundle de otra versión de la app.
