...
| 27 | Forma del payload para **fijar** un temporizador (`temporizer`) o un programa/horario (`programs`) (F6) | 🟢 Descartado por decisión de alcance (2026-07-21) | **Solo el nombre y la dirección de los comandos están confirmados** en el bundle (`setTemporizer(e,t,i,...)` → `command:"temporizer"`, `value:i`; `getPrograms(e,t,...)` → `command:"programs"`, `value:""`) — la forma del `value` para `temporizer` nunca se decodifica en el bundle (`i` se pasa opaco, sin desglose de campos), y **no existe ningún `setPrograms`/equivalente de escritura** en el bundle extraído, solo lectura. El campo de solo-lectura `tzv` (valor de temporizador actual) está confirmado como parte del wire, pero tampoco se parsea hoy en `status_parser.py` (no alimenta nada). Implementar el guardado de un temporizador o programa exigiría inventar la forma del payload, contra la regla del proyecto de no inventar campos (`CLAUDE.md` §6.4). **Decisión (confirmada con el usuario, ver `execution-plan.md` Tarea 27):** no se desarrolla F6 — igual que #15 (multi-location), es una decisión de alcance, no un bug ni una incógnita pendiente de investigar más. Solo se retomaría con una captura real de producción de una instalación con temporizador/programa configurado. |
...
- 🟢 **Aceptados por decisión de alcance:** #15 (multi-location — fuera de alcance, validado en producción con una `Location`); #27 (temporizador/programas, F6 — payload de escritura sin decodificar en el bundle, no se desarrolla).
